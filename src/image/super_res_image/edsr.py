...
class EDSR(SuperResolution, ABC):

    def __init__(self):
        super().__init__()
        self._log = Log(self.__class__.__name__)
        self._model_path = 'models/EDSR_x'
    ...
    def resize(self, original_image: Image, scaling_factor: int) -> Image | None:
        ...
        result_image.extend_filename(f'_sr_dnn_edsr_fac_{scaling_factor}')
        return result_image
    ...
    def _resize_cv(self, original_image: Image, scaling_factor: int) -> Image:
        ...
        superres.setModel('edsr', scaling_factor)
