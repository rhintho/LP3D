...
class Lanczos(SuperResolution, ABC):
    ...
    def resize(self, original_image: Image, scaling_factor: int) -> Image:
        ...
        new_image.extend_filename(f"_sr_lanczos_fac_{scaling_factor}")
        return new_image

    def _resize_cv(self, original_image: Image, new_width: int, new_height: int) -> Image:
        res = cv.resize(original_image.get_img(), (new_width, new_height), interpolation=cv.INTER_LANCZOS4)
        new_image = Image()
